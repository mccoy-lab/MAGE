...
def main():
	...
	fullResultsDF = pd.DataFrame({'GWAS_catalog_accessionID': pd.Series(dtype='str'),
								  ...
								  'sQTL_intronID': pd.Series(dtype='str'),
								  'sQTL_gene_ensemblID': pd.Series(dtype='str'),
								  'sQTL_gene_geneSymbol': pd.Series(dtype='str'),
								  'sQTL_variantChrom': pd.Series(dtype='str'),
								  'sQTL_variantPosition': pd.Series(dtype=int),
								  'sQTL_variantRef': pd.Series(dtype='str'),
								  'sQTL_variantAlt': pd.Series(dtype='str'),
								  'sQTL_variant_kgpID': pd.Series(dtype='str'),
								  'sQTL_variant_rsID': pd.Series(dtype='str'),
								  'sQTL_geneCS': pd.Series(dtype='str'),
	...
	sQTL_CS_DF = pd.DataFrame({'GWAS_catalog_accessionID': pd.Series(dtype='str'),
							   'traitDescription' : pd.Series(dtype='str'),
							   'GWAS_sentinelVariant' : pd.Series(dtype='str'),
							   'variantChrom' : pd.Series(dtype='str'),
							   'variantPosition' : pd.Series(dtype=int),
	...
	with open(args.mapFile) as in_fs:
		for line in in_fs:
			...
			traitDescription = fields[1]
			sentinelVariant = fields[2]
			fpath = fields[3] + ".coloc.sQTL_CS.txt"

			if os.path.exists(fpath):
				signalsQTL_CS_DF = pd.read_csv(fpath, header=0, sep='\t')
				signalsQTL_CS_DF['GWAS_catalog_accessionID'] = traitID
				signalsQTL_CS_DF['traitDescription'] = traitDescription
				signalsQTL_CS_DF['GWAS_sentinelVariant'] = sentinelVariant
				signalsQTL_CS_DF = signalsQTL_CS_DF.loc[:, sQTL_CS_DF.columns]

				sQTL_CS_DF = pd.concat([sQTL_CS_DF, signalsQTL_CS_DF], axis=0)

	sQTL_CS_DF = sQTL_CS_DF.sort_values(by=['GWAS_catalog_accessionID'])
	sQTL_CS_DF.to_csv(f'{args.outPrefix}.moderate_sQTL_CS.txt', header=True, index=False, sep='\t', na_rep='NA')
