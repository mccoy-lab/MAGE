...
p <- arg_parser("Run SuSiE eQTL fine-mapping for a single gene")
p <- add_argument(p, '--geneID', short='-g', help='ID of gene to run fine-mapping for')
...
p <- add_argument(p, '--expressionBed', short='-e', help='Bed file with expression values for each sample')
...
cat('\nRunning SuSiE for gene: ', args$geneID, '\n')
...
expression_data <- read.table(args$expressionBed, sep='\t', header=TRUE, comment.char="")
expression_data <- expression_data[,4:ncol(expression_data)]
Y <- as.numeric(expression_data[expression_data[,1]==args$geneID, sample_ids])
names(Y) <- sample_ids
...
cat('\t- Writing output\n')
snp_output_file <- paste0(args$outDir, "/", args$geneID, "_full_pip_results.txt")
cs_output_file <- paste0(args$outDir, "/", args$geneID, "_cs_results.txt")
