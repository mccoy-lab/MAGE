...
def main():
	...
	print(f'\nRunning multi-variant interaction model for {nAssociations} gene-eQTL associations', flush=True)
	print(f'\t0/{nAssociations} tests completed', end='\r', flush=True)

	counter = 0
	for geneID_clean in eqtl_df['gene_id_clean'].unique():
		gene_eqtl_df = eqtl_df.loc[eqtl_df['gene_id_clean']==geneID_clean]
		
		base_formula = f"{geneID_clean} ~ population + " + ' + '.join([variantID_clean for variantID_clean in gene_eqtl_df['variant_id_clean']]) + ' + ' + ' + '.join(cov_list)

		for i, row in gene_eqtl_df.iterrows():
			geneID = row['gene_id']
			...
			if min(sorted(variant_AFs)[-2:]) < args.maf_threshold:
				...
				print(f'\t{counter+1}/{nAssociations} tests completed', end='\r', flush=True)
				counter += 1
				continue

			formula_1 = base_formula
			formula_2 = f'{base_formula} + {variantID_clean}:population'
	...
	print(f'\t{counter}/{nAssociations} tests completed\n\nAll done!\n')
